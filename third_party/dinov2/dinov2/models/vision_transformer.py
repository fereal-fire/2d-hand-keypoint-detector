...
class DinoVisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        assert N == h0 * w0, f"DINOv2 expects pos_embed N={N} to match input grid {h0}x{w0}={h0*w0}"

        pos = pos.reshape(1, h0, w0, D).permute(0, 3, 1, 2)   # (1,D,h0,w0)
        pos = torch.nn.functional.interpolate(
            pos, size=(h0, w0), mode="bicubic",
            align_corners=False, antialias=self.interpolate_antialias
        )
        pos = pos.permute(0, 2, 3, 1).reshape(1, h0 * w0, D)

        return torch.cat((cls_pos, pos), dim=1).to(previous_dtype)
