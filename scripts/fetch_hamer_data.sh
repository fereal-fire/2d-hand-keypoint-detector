...
ARCHIVE_DIR="${HAMER_ROOT}/raw_archives"
...
echo
echo "Downloading HaMeR training archives..."
...
echo
echo "Extracting HaMeR archives to ${HAMER_ROOT}..."

for f in hamer_training_data_part*.tar.gz; do
    tar --warning=no-unknown-keyword --exclude=".*" -xvf $f -C "${HAMER_ROOT}"
done

echo "Hamer data downloaded."